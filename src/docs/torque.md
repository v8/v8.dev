...
Union types express that a value belongs to one of several possible types. We only allow union types for tagged values, because they can be distinguished at runtime using the map pointer.
For example, JavaScript numbers are either Smi values or allocated HeapNumber objects.
...
Every Torque `macro` produces a CSA-generating function. When compiled by the Torque compiler, macros generate a bunch of C++ code a `*-gen.cc` file in the build’s output directory. This code looks just like other code that you might find in `code-stub-assembler.cc`, albeit a bit less readable because it’s machine-generated.

<pre><code class="language-grammar">MacroDeclaration :
  <b>extern macro</b> IdentifierName ImplicitParameters<sub>opt</sub> ExplicitParameters ReturnType<sub>opt</sub> LabelsDeclaration<sub>opt</sub>
...
Torque modules are actually more like namespaces. They allow you to create declarations that are not implicitly visible in other namespaces. Modules can be nested, and declarations inside a nested module can see the declarations in the module that contains them without qualification. Declarations that are explicitly in a module declaration are in a shared global module. Modules can be reopened, allowing modules to be defined over multiple files.

```torque
macro IsJSObject(o: Object): bool { … }
...
This can be solved using something similar to [Scala’s implicit parameters](https://docs.scala-lang.org/tour/implicit-parameters.html). Concretely:
A macro can declare implicit parameters in addition to explicit ones:
