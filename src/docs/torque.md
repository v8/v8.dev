...
For example, the following module declartion specifies two Torque `macro`s in the `foo` module (`macro`s are a Torque primitive that specifies how to generate a bunch of CSA code and they will be discussed in more detail below, but for this example if suffices to know that they produce CSA-generating C++ code that would have been hand-written in a method of a `CodeStubAssembler` class in a pre-Torque world):
...
<pre><code class="language-grammar">MacroDeclaration :
  <b>macro</b> IdentifierName ImplicitParameters<sub>opt</sub> ExplicitParameters ReturnType<sub>opt</sub>
    LabelsDeclaration<sub>opt</sub> StatementBlock
  <b>extern macro</b> IdentifierName ImplicitParameters<sub>opt</sub> ExplicitTypes ReturnType<sub>opt</sub>
