...
<figure>
  <img src="/_img/cost-of-javascript-2019/reddit-js-processing.svg" intrinsicsize="1280x774" alt="JS processing represents 10-30% of time spent in V8 during page load">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/reddit-js-processing-devices.svg" intrinsicsize="1280x774" alt="Cost of Reddit JavaScript across a few different device classes (low-end, average and high-end)">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/long-tasks.png" srcset="/_img/cost-of-javascript-2019/long-tasks@2x.png 2x" intrinsicsize="1280x774" alt="Long tasks monopolize the main thread. You should break them up.">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/chrome-js-parse-times.svg" intrinsicsize="1280x836" alt="V8 parse times across different versions">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/js-parse-times-websites.svg" intrinsicsize="1280x774" alt="In the time it took Chrome 61 to parse Facebook's JS, Chrome 75 can now parse both Facebook's JS and 6 times Twitter's JS">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/script-streaming-1.svg" intrinsicsize="1280x774" alt="Scripts arrive in multiple chunks. V8 starts streaming once it's seen at least 30KB.">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/script-streaming-2.svg" intrinsicsize="1280x774" alt="Chrome 71 moved to a task-based setup where the scheduler could parse multiple async/deferred scripts at once">
</figure>
...
[Previous versions of Chrome](https://v8.dev/blog/v8-release-75#script-streaming-directly-from-network) supported streaming parsing and compilation where the script source data coming in from the network had to make its way to Chrome's main thread before it would be forwarded to the streamer.
...
<figure>
  <img src="/_img/cost-of-javascript-2019/devtools-69.png" srcset="/_img/cost-of-javascript-2019/devtools-69@2x.png 2x" intrinsicsize="931x98" alt="Previously there was an issue in DevTools that rendered the entire parser task in a way that hints that it's using CPU (full block)">
</figure>
...
<figure>
  <img src="/_img/cost-of-javascript-2019/devtools-76.png" srcset="/_img/cost-of-javascript-2019/devtools-76@2x.png 2x" intrinsicsize="922x441" alt="In Chrome 76, parsing is broken up into multiple smaller streaming tasks.">
  <figcaption>In Chrome 76, parsing is broken up into multiple smaller streaming tasks.</figcaption>
</figure>

In general, the DevTools performance pane is great for getting a high-level overview of what's happening on your page. For detailed V8-specific metrics such as JavaScript parse and compile times, we recommend [using Chrome Tracing with Runtime Call Stats (RCS)](https://v8.dev/docs/rcs). In RCS results, `Parse-Background` and `Compile-Background` tell you how much time was spent parsing and compiling JavaScript off the main thread, whereas `Parse` and `Compile` captures the main thread metrics.
...
<figure>
  <img src="/_img/cost-of-javascript-2019/reddit-main-thread.svg" intrinsicsize="1280x774" alt="Main thread vs worker therad time spent parsing and compiling Reddit's JS on a Macbook Pro">
</figure>

Reddit.com has several 100 kB+ bundles which are wrapped in outer functions causing lots of [lazy compilation](https://v8.dev/blog/preparser) on the main thread. In the above chart, the main thread time is all that really matters because keeping the main thread busy can delay interactivity. Reddit spends most of its time on the main thread with minimum usage of the Worker/Background thread.
...
<figure>
  <img src="/_img/cost-of-javascript-2019/facebook-main-thread.svg" intrinsicsize="1280x774" alt="Main thread vs worker therad time spent parsing and compiling Facebook's JS on a Macbook Pro">
</figure>
...
The first pass can't be avoided. Luckily, the second pass can be avoided by placing the object literal at the top-level, or within a [PIFE](https://v8.dev/blog/preparser#pife).
